...
HipKneeTrain$Cluster <- as.factor(kmeans_result$cluster)

# Cluster characteristics
cluster_summary <- HipKneeTrain %>%
  group_by(Cluster) %>%
...
for (feature in features_to_plot) {
  p <- ggplot(HipKneeTrain, aes(x = .data$Cluster, y = .data[[feature]], fill = .data$Cluster)) +
    geom_boxplot() +
    theme_minimal() +
...
# Add cluster assignments to HipKneeTrain
HipKneeTrain$HC_Cluster <- as.factor(hc_clusters)
...
hc_cluster_summary <- HipKneeTrain %>%
  group_by(HC_Cluster) %>%
  summarise_if(is.numeric, mean, na.rm = TRUE)
...
for (feature in features_to_plot) {
  p <- ggplot(HipKneeTrain, aes_string(x = "HC_Cluster", y = feature, fill = "HC_Cluster")) +
    geom_boxplot() +
    theme_minimal() +
...
# Train RF model
rf_model <- randomForest(PredictedReadmissionRate_HIP_KNEE ~ ., data = HipKneeTrain, importance = TRUE, na.action = na.omit)
