...
### Loading the Data (AC)  
Note: Data must be saved to personal computer and the filepath must be specified for each user due to the size of the files. The data can be accessed [here](https://data.cms.gov/provider-data/topics/hospitals). Click "download all datasets". 
```{r, message=FALSE, warning=FALSE}
# Set the directory for the data files - must be saved to personal computer (too large for GitHub). Can be accessed here: https://data.cms.gov/provider-data/topics/hospitals
...
non_numeric_columns <- c("State", "FacilityName", "FacilityId", "StateCode")
numeric_columns <- setdiff(names(HipKneeTrain), non_numeric_columns)

# Scale and normalize the numeric columns
HipKneeTrain_scaled <- as.data.frame(scale(HipKneeTrain[, numeric_columns]))

# Combine the scaled data with the non-numeric data
HipKneeTrain <- cbind(HipKneeTrain_scaled, HipKneeTrain[, non_numeric_columns])

# Print the first few rows of the scaled and normalized dataset
head(HipKneeTrain)
```
...
#### Removing columns correlated with the target variable
Score_COMP_HIP_KNEE and Payment_PAYM_90_HIP_KNEE are highly correlated with the target variable, so they will be removed. 
```{r}
# Removing correlated columns
HipKneeTrain <- HipKneeTrain %>%
  select(-Score_COMP_HIP_KNEE, -Payment_PAYM_90_HIP_KNEE)
```

#### Saving the dataset for future ease of use
```{r}
...
### Loading the Data 
Note: Data must be saved to personal computer and the filepath must be specified for each user due to the size of the files. The data can be accessed [here](https://data.cms.gov/provider-data/archived-data/hospitals). Download the snapshot from 4/24/2024. 
```{r, message=FALSE, warning=FALSE}
# Set the directory for the data files - must be saved to personal computer (too large for GitHub). Can be accessed here: https://data.cms.gov/provider-data/archived-data/hospitals
...
# Exclude non-numeric columns
non_numeric_columns <- c("State", "FacilityName", "FacilityId", "StateCode")
numeric_columns <- setdiff(names(HipKneeTest), non_numeric_columns)

# Scale and normalize the numeric columns
HipKneeTest_scaled <- as.data.frame(scale(HipKneeTest[, numeric_columns]))

# Combine the scaled data with the non-numeric data
HipKneeTest <- cbind(HipKneeTest_scaled, HipKneeTest[, non_numeric_columns])
...
descr_stats <- descr_stats %>% filter(vars != c("FacilityId", "State", "StateCode", "FacilityName"))
# Remove columns 1, 2, 5, and 6
descr_stats <- descr_stats[, -c(1, 2, 5, 6)]

# Create a table with kable
desc_stats_table <- kable(descr_stats, format = "html", caption = "Table 2. Descriptive Statistics for All Numeric Variables in Final Dataset") %>%
  kable_styling(
...
features_to_plot <- c("PredictedReadmissionRate_HIP_KNEE", "HcahpsLinearMeanValue_H_HSP_RATING_LINEAR_SCORE", "SAFE_USE_OF_OPIOIDS")
...
HipKneeTrain_QualRF <- HipKneeTrain_Qual %>%
  select(-State, -StateCode, -FacilityName, -FacilityId, -PredictedReadmissionRate_HIP_KNEE)
...
# Print best mtry parameters
best_mtry <- rf_grid_search_mtry$bestTune
print("Best mtry parameters:")
print(best_mtry)

# Optimize ntree using the best mtry
grid_ntree <- c(5, 10, 12)

# Define the grid for mtry with the best mtry value
best_mtry_grid <- expand.grid(mtry = best_mtry$mtry)

# Train the Random Forest model with grid search for ntree
rf_results <- lapply(grid_ntree, function(ntree_val) {
  train(
    TargetCategory ~ .,   
    data = HipKneeTrain_QualRF,                    
    method = "rf",                          
    trControl = train_control,              
    tuneGrid = best_mtry_grid,
    importance = TRUE,                     
    ntree = ntree_val                         
  )
})

# Find the best ntree based on performance metrics
best_ntree_index <- which.max(sapply(rf_results, function(model) max(model$results$Accuracy)))
best_rf_model_qual <- rf_results[[best_ntree_index]]
best_ntree <- grid_ntree[best_ntree_index]
...
# Extract feature importances
feature_importances_qual <- importance(best_rf_model_qual$finalModel)
...
sorted_by_importance_qual <- feature_importances_df_qual[order(-feature_importances_df_qual$MeanDecreaseAccuracy), ]

# Print feature importances
print("Feature importances:")
print(sorted_by_importance_qual)

# Graph top 10 feature importances 
top10_features <- data.frame(
  Feature = c(
    "Score_PSI_15", "Score_PSI_14", "Score_PSI_10", 
    "H_HSP_RATING_LINEAR_SCORE", 
    "SAFE_USE_OF_OPIOIDS", "EDV", "Score_PSI_11", 
    "Score_PSI_12", "OP_29", "HCP_COVID_19"
  ),
  MeanDecreaseAccuracy = c(
    3.85, 3.00, 2.38, 2.20, 2.12, 2.07, 
    2.04, 2.02, 1.51, 1.10
  )
)
...
# Plot
ggplot(top10_features, aes(x = reorder(Feature, MeanDecreaseAccuracy), y = MeanDecreaseAccuracy, fill = MeanDecreaseAccuracy)) +
  geom_bar(stat = "identity", color = "black") +
  coord_flip() +
  theme_minimal() +
  scale_fill_gradientn(colors = c("lightblue", "blue", "darkblue")) +
  labs(
    title = "Figure 12. Top 10 Important Features from Random Forest",
    x = "Feature",
...
pred_rf_qual <- predict(best_rf_model_qual, newdata = HipKneeTest_QualRF)
...
# Predict on the test set for probability scores
pred_prob_rf_qual <- predict(best_rf_model_qual, newdata = HipKneeTest_QualRF, type = "prob")

# Extract the predicted probabilities for the positive class
pred_prob_pos <- pred_prob_rf_qual[, "1"]
...
plot(roc_curve, main = "Figure 11. ROC Curve for Random Forest Model", col = "blue")
...
# Separate predictors and response variable in the training set - removed StateCode to reduce overfitting and improve interpretability
x_train_qual <- HipKneeTrain_Qual %>% select(-c(State, StateCode, FacilityName, FacilityId, PredictedReadmissionRate_HIP_KNEE, TargetCategory))
...
# Separate predictors and response variable in the test set
x_test_qual <- HipKneeTest_Qual %>% select(-c(State, StateCode, FacilityName, FacilityId, PredictedReadmissionRate_HIP_KNEE, TargetCategory))
...
### Assessing performance
```{r}
# Predict on the test set with probability = TRUE
...
pred_prob <- attr(pred_test_svm_tuned, "probabilities")[,2]

# Calculate the ROC curve using the predicted probabilities
pred <- prediction(pred_prob, y_test_qual)
roc_curve <- performance(pred, "tpr", "fpr")
...
```{r}
# Load tensorflow (loaded here due to package conflicts)
...
x_train <- HipKneeTrain_Qual %>%
  select(-State, -FacilityName, -StateCode, -FacilityId, -PredictedReadmissionRate_HIP_KNEE, -TargetCategory) %>%
  as.matrix()

y_train <- keras::to_categorical(HipKneeTrain_Qual$TargetCategory, num_classes = 2)

x_test <- HipKneeTest_Qual %>%
  select(-State, -StateCode, -FacilityName, -FacilityId, -PredictedReadmissionRate_HIP_KNEE, -TargetCategory) %>%
  as.matrix()

y_test <- keras::to_categorical(HipKneeTest_Qual$TargetCategory, num_classes = 2)
...
# Define early stopping callback (to avoid redundancy)
...
# Initialize HipKneeTest_Qual_nn as a copy of HipKneeTest_Qual
...
print("df with nn predictions:")
print(head(HipKneeTest_Qual_nn))
```

Note: Code successfully self-tested and user-tested as of 8/23/2024. 
